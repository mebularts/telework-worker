...
async function scrape() {
    ...
    for (const source of SOURCES) {
        try {
            ...
            try {
                await page.waitForSelector(source.containerSelector, { timeout: 10000 });
            } catch (e) {
                console.warn(`Timeout waiting for ${source.containerSelector} on ${source.name}, proceeding anyway...`);
            }
            ...
            const threads = await page.evaluate((s) => {
                ...
                nodes.forEach(el => {
                    ...
                    // Clean the title from potential prefix/badge text if needed
                    // But for now, simple trim is ok.

                    if (title && url) {
                        // Ensure it's a forum link
                        if (url.includes('.html') || url.includes('/forum/') || url.includes('thread')) {
